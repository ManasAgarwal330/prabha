...
<template>
  <ol class="relative border-l border-line pl-8 sm:pl-10">
    <li v-for="day in days" :key="day.day" class="relative pb-10 last:pb-0">
      <span
        class="absolute -left-[2.3rem] flex h-8 w-8 items-center justify-center rounded-pill border border-line bg-ivory text-[0.7rem] font-medium text-charcoal-muted sm:-left-[2.8rem]"
        aria-hidden="true"
      >
      ...
      <p class="mt-2.5 max-w-prose leading-relaxed text-charcoal-muted">{{ day.description }}</p>

      <div v-if="day.stay || day.meals" class="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs text-charcoal-muted">
